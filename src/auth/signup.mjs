...
export async function register() {
    ...
    try {
        ...
        } else {status.innerHTML ="The user already exist or something went bad on the server side"};
    }
...
function validateUserInputsSignUp (data) {
    ...
    let regEmail = /^[a-zA-Z0-9._%+-]+@(?:stud.noroff|noroff).no/;
    let validateEmail = regEmail.test(data.email);
    console.log(validateEmail);
    if(!validateEmail) {
        errorEmail.innerHTML ="not a valid email (use @stud.noroff.no/noroff.no)."
